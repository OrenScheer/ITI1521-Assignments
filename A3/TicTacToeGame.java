...
public class TicTacToeGame {
	...
	*/
	...
	private int numTransformationsPerformed; // Instance variable to keep track of current state (how many transformations have been performed since last reset call)
    ...
    *  the number of cells that must be aligned to win.
	...
	public TicTacToeGame(int lines, int columns, int sizeWin){
		...
		transformedBoard = new int[board.length]; // Create and initialize board to reflect unchanged game
		for (int i = 0; i < transformedBoard.length; i++) {
			transformedBoard[i] = i;
		}
		numTransformationsPerformed = 0; // 0 originally preformed
	}
    ...
    *  the index of the next move.
	...
	public TicTacToeGame(TicTacToeGame base, int next){
		...
		// UPDATE HERE IF NEEDED
		transformedBoard = new int[board.length];
		for (int i = 0; i < transformedBoard.length; i++) {
			transformedBoard[i] = i;
		}
		numTransformationsPerformed = 0;
	}
    ...
    *  String representation of the game
    ...
    public boolean hasNext(){
    	...
    	// YOUR CODE HERE
			return  (lines == columns && numTransformationsPerformed < 8) || (lines != columns && numTransformationsPerformed < 4); // Square is done if 8 have been performed, rectangle is done if 4 have been performed
    }
    ...
    public void next(){
    	...
    	// YOUR CODE HERE
			if (!hasNext()) { // Max number of transformations for square/non-square boards
				throw new IllegalStateException("All states have already been checked.");
			}

			if (lines == columns) { // If square
				if (numTransformationsPerformed == 0) {
					transform(Transformation.ID);
				}
				else if (numTransformationsPerformed < 4) {
					transform(Transformation.ROT);
				}
				else if (numTransformationsPerformed == 4) {
					transform(Transformation.HSYM);
				}
				else {
					transform(Transformation.ROT);
				}
				...
				else if (numTransformationsPerformed == 1) {
					transform(Transformation.HSYM);
				}
				else if (numTransformationsPerformed == 2){
					transform(Transformation.VSYM);
				}
				else {
					transform(Transformation.HSYM);
				}
			}
			numTransformationsPerformed++;
    }

		/**
     * Helper function that applies the transformation associated with each
		 * of the Transformation enums.
     * @param t
		 *  transformation to be applied
     */
		private void transform(Transformation t) {
			switch (t) {
				case ID:
					reset();
					break;
				...
				case HSYM:
					...
					break;
				default:
					throw new IllegalArgumentException("Illegal transformation.");
			}
		}
    ...
    *  the TicTacToeGame instance to be compared with this one
  	*/
  	public boolean equalsWithSymmetry(TicTacToeGame other){
  		...
  		// YOUR CODE HERE
			numTransformationsPerformed = 0; // Reset as this will  not already be 0 if board was previously compared
			while (hasNext()) { // While there are still symmetries left
				next();
				boolean same = true;
				for (int i = 0; i < transformedBoard.length; i++) {
					if (board[transformedBoard[i]] != other.valueAt(i)) { // Compare the indirection of the instance game with the other game
						same = false;
						break;
					}
				}
				if (same) {
					return true; // If they were identical, the games are logically equivalent
				}
			}
			return false;
    }
